...
const getStreak = async (req, res, next) => {
  try {
    const user = await prisma.user.findUnique({
      ...
      select: {
        currentStreak: true, longestStreak: true, lastReadDate: true,
        totalXp: true, badges: true, totalReadingMinutes: true, readingGoal: true,
      },
    });
...
const updateStreak = async (userId, { minutesRead = 0, isCompleted = false, readingLanguage = 'English', hour = new Date().getHours(), startedAt = null } = {}) => {
  try {
    ...
    let   xpGain         = Math.floor(minutesRead / 5);
    ...
    if (isCompleted) {
      ...
      const completedCount = await prisma.userProgress.count({ where: { userId, isCompleted: true } });
      if (completedCount >= 10) earnedBadges.add('book_worm');
      if (completedCount >= 25) earnedBadges.add('scholar');
      if (startedAt && (Date.now() - new Date(startedAt).getTime()) < 24 * 60 * 60 * 1000) {
        earnedBadges.add('speed_reader');
      }
    }
