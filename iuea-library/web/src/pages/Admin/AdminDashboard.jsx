...
const STAT_CARDS = [
  { key: 'books',     label: 'Total Books',      icon: 'menu_book', delta: '+12%',  deltaColor: '#B8964A', deltaBg: 'rgba(201,168,76,0.1)',  fallback: '42,892' },
  { key: 'users',     label: 'Active Users',      icon: 'person',    delta: '+4.2k', deltaColor: '#5C0F1F', deltaBg: 'rgba(107,15,26,0.1)',   fallback: '12,450' },
  { key: 'downloads', label: 'Downloads Today',   icon: 'download',  delta: '-2%',   deltaColor: '#ba1a1a', deltaBg: 'rgba(186,26,26,0.1)',   fallback: '1,830'  },
  { key: 'podcasts',  label: 'Podcast Plays',     icon: 'podcasts',  delta: '+22%',  deltaColor: '#B8964A', deltaBg: 'rgba(201,168,76,0.1)',  fallback: '5,204'  },
];

const ACTIVITY = [
  { icon: 'add_circle',  iconColor: '#5C0F1F', iconBg: 'rgba(107,15,26,0.1)',  title: 'New Book Cataloged',    desc: '"The Architecture of Thought" by Dr. Julian Vane',   time: '2 MINS AGO'   },
  { icon: 'person_add',  iconColor: '#B8964A', iconBg: 'rgba(201,168,76,0.1)', title: 'New Student Joined',    desc: 'Sarah Jenkins (Faculty of Engineering)',              time: '15 MINS AGO'  },
  { icon: 'play_circle', iconColor: '#5C0F1F', iconBg: 'rgba(107,15,26,0.1)',  title: 'Trending Podcast',       desc: '"Ethics in AI" reached 500+ streams',                time: '1 HOUR AGO'   },
  { icon: 'history',     iconColor: '#8A1228', iconBg: '#ffd9dc',              title: 'System Audit',           desc: 'Weekly database synchronization completed',          time: '3 HOURS AGO'  },
];
      ...
      <div style={{ padding: '2rem', display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              {recentUsers.length > 0 ? recentUsers.map((u) => (
                ...
                <div key={i} style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.875rem', fontWeight: 600, color: '#1C0A0C', margin: 0 }}>{a.title}</p>
                    <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.75rem', color: '#6B5456', margin: 0 }}>{a.desc}</p>
                    <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '10px', color: 'rgba(138,18,40,0.5)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', margin: 0 }}>{a.time}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section style={{ paddingBottom: '3rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '1.5rem' }}>
            ...
            <a href="#" style={{ fontSize: '0.875rem', fontWeight: 500, color: '#8A1228', textDecoration: 'none', fontFamily: 'Inter, sans-serif' }}
          ...
          <div className="ad-catalog-grid">
            {CATALOG.map((c, i) => (
              <div key={i} className="ad-catalog-card" style={{ transform: c.offset ? 'translateY(2rem)' : 'none' }}>
                ...
                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(138,18,40,0.8) 0%, transparent 60%)', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '1.5rem' }}>
